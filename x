...
local function isWithinBoxFOV(position)
    ...
    local fovHeight = getgenv().Forbidden.BoxFov.Height * 100
    local fovWidth = getgenv().Forbidden.BoxFov.Width * 100
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode[string.upper(getgenv().Forbidden.KeyTrigger.Visuals)] then
...
local function updateTracer()
    if TargetPlayer and TargetPlayer.Character then
        ...
        if getgenv().Forbidden.Visuals.TargetClosestBodyPart then
        ...
        if targetPart then
            if not TracerLine then
                ...
                TracerLine.Thickness = getgenv().Forbidden.Visuals.TracerThickness
                TracerLine.Transparency = getgenv().Forbidden.Visuals.TracerTransparency
            end
...
local function handleESP()
    if visualsActive and getgenv().Forbidden.Visuals.Enabled then
...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = getgenv().Forbidden.CamLock.Normal.Radius_Visibility
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if getgenv().Forbidden.CamLock.Normal.Aligner then
            ...
            if getgenv().Forbidden.Silent.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                if getgenv().Forbidden.Silent.mode == "target" then
                    local closestPoint
                    if getgenv().Forbidden.Silent.TargetMode == "OptimalTargetPoint" then
                        closestPoint = GetOptimalTargetPoint(TargetPlayer.Character)
                    elseif getgenv().Forbidden.Silent.TargetMode == "Closest Point" then
                        closestPoint = GetClosestPoint(TargetPlayer.Character)
                    elseif getgenv().Forbidden.Silent.TargetMode == "BasicTargeting" then
